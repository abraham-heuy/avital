...
export const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Registration successful! A verification code has been sent to your email and WhatsApp.');
      setTimeout(() => navigate('/verify', { state: { email: formData.email, type: 'verify' } }), 1500);
    } catch (err: any) {
  ...
  return (
    <div
      className="relative min-h-screen bg-rb-black overflow-hidden flex items-center justify-center py-10"
      ...
      <div className="relative z-10 w-full max-w-md px-4">
        ...
        >
          <div className="text-center mb-8">
            ...
            <p className="text-rb-gray text-sm mt-2">Create an account</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <select
                ...
                onChange={handleChange}
                className="w-full px-0 py-2 bg-transparent border-b border-rb-silver/30 text-rb-silver text-sm focus:border-rb-blue focus:outline-none transition-all"
              >
                <option value="" className="bg-rb-black">Select year</option>
                {yearOptions.map((year) => (
                  <option key={year} value={year} className="bg-rb-black">
                    {year}
                  </option>
                ))}
              </select>
